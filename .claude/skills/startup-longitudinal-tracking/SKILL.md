...
- The commands, from the Kotlin `embrace-analysis-*` modules (run as `tools/startup <command>` from the
  repo root; `embrace-analysis-cli/README.md` has the table, `tools/startup <command> --help` the flags;
...
3. `tools/startup reference-set --probe --out reference-set.json` records each device's
   ...
   stable `device_key` you choose (e.g. `entry-a`, `mid-b`) that is used in every later report. The
   serials go to the machine-local `_shared/local/data/device-serials.json`, never into the committed
   file: a serial names a handset on one machine, a key names it in every record. Rename a key in
   both files together.
