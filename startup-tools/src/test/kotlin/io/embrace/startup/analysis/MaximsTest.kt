...
class MaximsTest {
    ...
    fun `an arm that clears app data before every launch cannot test the config fast path`() {
        ...
        val out = MaximsCommand.score(campaign, null, "mid-a", "9.2.0", "new-user-session", "none", null, NOW_A)
        ...
        val plain = MaximsCommand.score(campaign.parent.resolve("campaign-a-plain"), null, null, null, null, "none", null, NOW_A)
    ...
    fun `a single-cohort arm cannot test restore against create`() {
        ...
        val out = MaximsCommand.score(campaign, null, "mid-a", "9.2.0", "expired-user-session", "none", null, NOW_A)
