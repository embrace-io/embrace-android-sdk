...
import io.embrace.android.embracesdk.internal.otel.sdk.sanitizeAttributesMap
...
private class EmbraceSpanImpl(
    ...
    private fun populateEvents(spanToStop: Span) {
        ...
        (systemEvents + redactedCustomEvents).forEach { event ->
            val eventAttributes = event.attributes.sanitizeAttributesMap(otelSpanCreator.spanStartArgs.internal, dataValidator)
    ...
    private fun populateLinks(spanToStop: Span) {
        ...
        (systemLinks + redactedCustomLinks).forEach {
            val linkAttributes = it.attributes.sanitizeAttributesMap(false, dataValidator)
