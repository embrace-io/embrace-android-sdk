...
internal class ExperimentTrackingTest {
    ...
    @Test
    fun `experiment state is attached to session part spans and logs in the happy path`() {
        testRule.runTest(
            preSdkStartAction = {
            ...
            assertAction = {
                ...
                // the first session part span carries the full state as of session end:
                ...
                assertEquals(
                    ...
                    sessions[0].findSessionPartSpan().experimentsBlob(),
                )

                // records persist for the entire process (they only age out when a new process
                // starts without re-tracking), so the second session part span carries the same
                // state, including exp1's closed record
                assertEquals(
                    "e:exp1:variantA:$preStartBucketTimeMs:$experimentEndTimeMs;" +
                        "e:exp2:variantB:$secondBucketTimeMs;" +
                        "f:flag1::$flagTrackTimeMs",
                    sessions[1].findSessionPartSpan().experimentsBlob(),
                )

                // the log carries the state as of log emission: both experiments and the feature
                // flag tracked, no end time on exp1 yet
                val log = getSingleLogEnvelope().getLogOfType(EmbType.System.Log)
                assertEquals(
                    "e:exp1:variantA:$preStartBucketTimeMs;" +
                        "e:exp2:variantB:$secondBucketTimeMs;" +
                        "f:flag1::$flagTrackTimeMs",
                    checkNotNull(log.attributes).toMap()[EMB_EXPERIMENTS_ATTRIBUTE_KEY],
                )
            },
        )
    }

    private fun Span.experimentsBlob(): String? =
        checkNotNull(attributes).toMap()[EMB_EXPERIMENTS_ATTRIBUTE_KEY]
}
