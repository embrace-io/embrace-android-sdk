...
**The rule: after any edit to a startup skill or to `startup-tools`, run the sweep before you call it
done.**

```
tools/startup check-leaks          # exits 1 on a hit, so it can gate a commit
tools/startup check-leaks --self-test   # checks the patterns themselves, scans nothing
```
...
credentials and scratch paths. Archives are read without unpacking, so a leak cannot hide inside one.
...
2. **If the hit is a legitimate look-alike**, suppress it where it sits, with the reason:
   `<!-- leakcheck:allow: the URL form, not a link to one -->`. A whole line carrying that marker is
...
The checked-in patterns describe the *shape* of a leak. The values one engineer's runs would leak, their
own serials and hosts, go one per line in `.leakcheck-local` at the repo root, which is git-ignored. Its
absence is reported as a notice, not a failure, and the generic layer still runs. Write your own when you
take the fleet over.

## It runs whether or not you remember

`LeakCheckTest` scans the repo as part of `./gradlew :startup-tools:test`, so a leak fails the suite the
same way a broken parity golden does. The command above is the fast way to see what it found.
