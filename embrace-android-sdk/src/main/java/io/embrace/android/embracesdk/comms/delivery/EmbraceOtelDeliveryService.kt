...
internal class EmbraceOtelDeliveryService(
    private val cacheManager: DeliveryCacheManager,
    private val otelExporter: ApiService,
    ...
    override fun sendSession(sessionMessage: SessionMessage, snapshotType: SessionSnapshotType) {
        ...
        val otelSignals = sanitizedSessionMessage.asOtelStandard()
        ...
        try {
            ...
            val future = otelExporter.sendSession(otelSignals) {
    ...
    private fun sendBackgroundActivityImpl(
        ...
        try {
            otelExporter.sendSession(action) {
    ...
    override fun sendLog(eventMessage: EventMessage) {
        otelExporter.sendLog(eventMessage)
    }

    override fun sendNetworkCall(networkEvent: NetworkEvent) {
        otelExporter.sendNetworkCall(networkEvent)
    }

    override fun sendCrash(crash: EventMessage, processTerminating: Boolean) {
        runCatching {
            cacheManager.saveCrash(crash)
            val future = otelExporter.sendCrash(crash)
    ...
    override fun sendAEIBlob(blobMessage: BlobMessage) {
        otelExporter.sendAEIBlob(blobMessage)
    }
        ...
        ids.forEach { id ->
            if (id != currentSession) {
                try {
                    ...
                    if (action != null) {
                        otelExporter.sendSession(action) { cacheManager.deleteSession(id) }
                    } else {
    ...
    override fun sendMoment(eventMessage: EventMessage) {
        sendSessionsExecutorService.submit {
            otelExporter.sendEvent(eventMessage)
        }
    }
}
