...
> **Editing this skill?** Run `tools/startup check-local-refs` before you call it done. Nothing here may
...
Primary analysis — the Kotlin `embrace-analysis-*` modules, run from the repo root as
`tools/startup <command>` (`embrace-analysis-cli/README.md` has the full command table;
...
- The SQL the commands execute lives in
  `embrace-analysis-reports/src/main/resources/io/embrace/analysis/reports/sql/`: `startup_metrics.sql` is
...
- `no-local-references.md` — **the sweep every edit to these skills ends with**: what may not describe
  ...
  legitimate look-alike. Enforced by `tools/startup check-local-refs` and by the tool's own test suite.
...
check, a scope, and a one-line mechanism, defined in the `embrace-analysis-*` modules and run as
...
- **Changing a belief.** A new maxim, a demotion or a refutation is a code change in
  the `embrace-analysis-*` modules, a dated entry in `maxims/FINDINGS.md` naming the campaign that did it, and a
