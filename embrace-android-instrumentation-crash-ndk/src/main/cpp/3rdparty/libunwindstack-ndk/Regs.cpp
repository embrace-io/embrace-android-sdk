...
uint64_t get_pc_adjustment_common(uint64_t rel_pc, Elf* elf, ArchEnum arch) {
  switch (arch) {
    case ARCH_ARM: {
      if (!elf->valid()) {
        return 2;
  ...
  case ARCH_UNKNOWN:
    ...
    uint64_t Regs::GetPcAdjustment(uint64_t rel_pc, Elf* elf, ArchEnum arch) {
        return get_pc_adjustment_common(rel_pc, elf, arch);
    }
