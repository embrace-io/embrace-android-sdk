...
internal class SpanSanitizer(
    ...
    private fun sanitizeSpans(span: EmbraceSpanData): Boolean {
        if (span.name == "emb-${FragmentBreadcrumbDataSource.SPAN_NAME}" && !shouldAddViewBreadcrumbs()) {
            return false
        }
        return true
    }
    ...
    private fun sanitizeEvents(event: EmbraceSpanEvent): Boolean {
        if (event.name == CustomBreadcrumbDataSource.EVENT_NAME && !shouldAddCustomBreadcrumbs()) {
            return false
        }
        return true
    }
