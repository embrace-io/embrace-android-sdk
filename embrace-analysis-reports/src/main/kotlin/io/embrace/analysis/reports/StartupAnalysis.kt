...
object StartupAnalysis {
    ...
    fun metricsOf(triples: List<TraceProcessor.Triple>): TraceMetrics {
        ...
        triples.forEach { t ->
            // A local copy: `value` is a property of another module, so the compiler will not smart-cast it.
            val value = t.value
            when {
                t.what == "window_source" -> source = t.k
                value == null -> Unit
                t.what == "section" -> sections[t.k] = value.toDouble()
                else -> scalars[t.what] = value.toDouble()
            }
        }
        return TraceMetrics(
            sections = sections,
    ...
    fun extract(tp: TraceProcessor, trace: Path): TraceMetrics =
        metricsOf(tp.triples(StartupQueries.STARTUP_METRICS, trace))
    ...
    fun fmtStats(vals: List<Double>): String {
        ...
        return "n=${vals.size}  min=${PyFormat.fixed(sorted.first(), 1)}  median=${PyFormat.fixed(Quantile.median(sorted), 1)}  " +
            "mean=${PyFormat.fixed(Descriptive.exactMean(vals), 1)}  max=${PyFormat.fixed(sorted.last(), 1)}"
    }
