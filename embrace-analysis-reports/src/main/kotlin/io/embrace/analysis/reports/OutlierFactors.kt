...
object OutlierFactors {
     ...
     * "no counter covered the window" - a distinction that matters because such nulls show up on most
     * iterations of one device whose cpufreq counter the kernel never exports.
     */
    ...
    fun extract(tp: TraceProcessor, trace: Path): Record =
        recordOf(trace.fileName.toString(), tp.triples(StartupQueries.OUTLIER_METRICS, trace))
}
