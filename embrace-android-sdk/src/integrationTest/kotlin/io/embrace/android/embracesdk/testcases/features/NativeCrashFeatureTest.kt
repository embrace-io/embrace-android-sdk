    ...
    fun `multiple native crashes can be associated with multiple sessions`() {
        testRule.runTest(
            ...
            assertAction = {
                ...
                val session1 = sessionEnvelopes.single { it.getSessionPartId() == crashData.nativeCrash.sessionPartId }
                session1.assertDeadPartResurrected(crashData)

                // sessions updated to include crash IDs
                val session2 = sessionEnvelopes.single { it.getSessionPartId() == crashData2.nativeCrash.sessionPartId }
