...
object VarianceAnalysis {
    ...
    fun extract(tp: TraceProcessor, trace: Path): Record =
        recordOf(trace.fileName.toString(), tp.triples(StartupQueries.VARIANCE_METRICS, trace))
    ...
    private fun StringBuilder.residency(data: List<Record>, windows: List<Double>, littleCpus: Set<Int>) {
        ...
        if (cl0Wins.isNotEmpty() && cl1Wins.isNotEmpty()) {
            line(
                "  little-majority: n=${cl0Wins.size} mean=${PyFormat.fixed(Descriptive.exactMean(cl0Wins), 1)}" +
                    "  |  cluster1-majority: n=${cl1Wins.size} mean=${PyFormat.fixed(Descriptive.exactMean(cl1Wins), 1)}",
            )
        }
    }
