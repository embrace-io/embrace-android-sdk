...
internal class AeiFeatureTest {
    ...
    fun `application exit info feature`() {
        ...
        with(testRule) {
            ...
            val deliveryService = harness.fakeDeliveryModule.deliveryService
            val payload = deliveryService.lastSentLogPayloads.single()

            // FIXME: this always is empty because ApplicationExitInfoService persists what has
            //  been sent. Ideally need to chain calls or delete AEI at same time as this PR.
            ...
            assertEquals(log.findLogAttribute("importance").toInt(), 125)
            ...
            assertEquals(log.findLogAttribute("status").toInt(), 1)
            assertEquals(log.findLogAttribute("description"), "testDescription")
            assertEquals(log.findLogAttribute("trace"), "testInputStream")
        }
    }
