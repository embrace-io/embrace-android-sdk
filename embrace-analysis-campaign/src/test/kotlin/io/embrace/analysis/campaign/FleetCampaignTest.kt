...
class FleetCampaignTest {
    ...
    fun `a dry run writes run-metadata and the campaign log without invoking gradle`() {
        ...
        val rc = FleetCampaign(
            ...
            gradle = { _, _, _ ->
                gradleCalls++
                io.embrace.analysis.common.proc.Processes.Output(0, "", "")
            },
            sleep = {},
            dryRun = true,
        ).run()
    ...
    fun `a pass that fails with the install-timeout signature is retried once, then aborts on a real failure`() {
        ...
        val outcomes = ArrayDeque(
            listOf(
                io.embrace.analysis.common.proc.Processes.Output(1, "... Failed to install split APK ...", ""),
                io.embrace.analysis.common.proc.Processes.Output(0, "BUILD SUCCESSFUL", ""),
                io.embrace.analysis.common.proc.Processes.Output(1, "some real failure", ""),
            ),
        )
    ...
    fun `benchmark methods map to the reference recipe's compile-state vocabulary`() {
        ...
        // CompilationMode.DEFAULT is the fresh-install state, not full AOT.
