...
internal class EmbraceSessionPropertiesTest {
    ...
    fun `Add session property with no error when maxSessionProperties is absent`() {
        Assert.assertTrue(sessionProperties.add(KEY_VALID, VALUE_VALID, false))
        Assert.assertEquals(1, sessionProperties.get().size.toLong())
    }
    ...
    fun addSessionProperty() {
        Assert.assertTrue(sessionProperties.add(KEY_VALID, VALUE_VALID, false))
        Assert.assertEquals(1, sessionProperties.get().size.toLong())
        Assert.assertEquals(VALUE_VALID, sessionProperties.get()[KEY_VALID])
        ...
        Assert.assertTrue(sessionProperties2.get().isEmpty())
    }

    @Test
    fun addSessionPropertyInvalidValue() {
        Assert.assertTrue(sessionProperties.get().isEmpty())
    }

    @Test
    fun addSessionPropertyPermanent() {
        Assert.assertTrue(sessionProperties.add(KEY_VALID, VALUE_VALID, true))
        Assert.assertEquals(1, sessionProperties.get().size.toLong())
        Assert.assertEquals(VALUE_VALID, sessionProperties.get()[KEY_VALID])

        // permanent property should have been persisted
        val sessionProperties2 = EmbraceSessionProperties(preferencesService, configService, logger)
        Assert.assertEquals(1, sessionProperties2.get().size.toLong())
        Assert.assertEquals(VALUE_VALID, sessionProperties2.get()[KEY_VALID])
        ...
        Assert.assertTrue(sessionProperties3.get().isEmpty())
    }
    ...
    fun addSessionPropertyFromMultipleThreads() {
        ...
        doneSignal.await()
        Assert.assertEquals(properties.size.toLong(), sessionProperties.get().size.toLong())
        Assert.assertEquals(expected, sessionProperties.get())
    }
    ...
    internal class AddPropertyWorker(
        ...
        override fun run() {
            try {
                startSignal.await()
                Assert.assertTrue(properties.add(property, VALUE_VALID, false))
    ...
    fun addPropertyTooManyWithDefaultMax() {
        ...
        for (i in 0 until MAX_SESSION_PROPERTIES_DEFAULT) {
            Assert.assertTrue(sessionProperties.add("prop$i", VALUE_VALID, isPermanent))
        ...
        Assert.assertTrue(sessionProperties.remove("prop0"))
        Assert.assertTrue(
    ...
    fun removeSessionProperty() {
        Assert.assertTrue(sessionProperties.add(KEY_VALID, VALUE_VALID, false))
        Assert.assertTrue(sessionProperties.remove(KEY_VALID))
        Assert.assertTrue(sessionProperties.get().isEmpty())
    }
    ...
    fun removeSessionPropertyInvalidKey() {
        Assert.assertFalse(sessionProperties.remove(""))
    }

    @Test
    fun removeSessionPropertyDoesNotExist() {
        Assert.assertFalse(sessionProperties.remove(KEY_VALID))
    }

    @Test
    fun removeSessionPropertyLongKey() {
        val longKey = "a".repeat(129)
        Assert.assertTrue(sessionProperties.add(longKey, VALUE_VALID, false))
        Assert.assertTrue(sessionProperties.remove(longKey))
        Assert.assertTrue(sessionProperties.get().isEmpty())
    }
}
