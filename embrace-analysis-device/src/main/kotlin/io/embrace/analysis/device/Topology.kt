...
data class Topology(
    ...
    /**
     * The one-line human summary the probe prints after writing [outPath]: identity, tier, memory and
     * storage class, cluster count, thermal sensors, and the `--little-cpus` value the analyses want.
     */
    fun summary(outPath: String): String {
        val homogNote = if (homogeneous) " (homogeneous — no DVFS cluster split detected, using policy0's cpus)" else ""
        val sensors = thermalSensors.takeIf { it.isNotEmpty() }?.joinToString(", ") ?: "none readable"
        val ram = ramMb?.let { "$it MB ($ramClass)" } ?: "RAM unknown"
        return "$name: ${vendor ?: "?"} ${model ?: "?"}, Android ${androidRelease ?: "?"} " +
            "(API ${apiLevel ?: "?"}); soc ${socFamily ?: "?"} ${socModel ?: ""}; " +
            "tier $tier; $ram; storage $storageClass; " +
            "${clusters.size} cpufreq policy(ies)$homogNote; " +
            "thermal sensors: $sensors. Wrote $outPath — pass " +
            "--little-cpus $littleCpusArg to variance, hypothesis-tests and factors-report " +
