...
import io.embrace.analysis.reports.OutlierFactors.Record
...
object FactorsReport {
        ...
        for (pi in 1..passCount) {
            ...
            line(
                "  ${pi.toString().padStart(W4)}" +
                    ...
                    f(Descriptive.exactMean(pr.map { it.sysserverRate }), 3, W13) +
                    f(Descriptive.exactMean(pr.map { it.sfRate }), 3, W9) +
                    f(Descriptive.exactMean(pr.map { it.otherRate }), 3, W11) +
                    f(Descriptive.exactMean(pr.map { it.artload }), 2, W9) +
                    f(Descriptive.exactMean(pr.map { it.dIo }), 2, W7) +
                    f(Descriptive.exactMean(pr.map { it.swap }) / MEGA, 1, W9),
            )
        }
        line()
    }
    ...
    private fun meanOrNan(values: List<Double>): Double =
        if (values.isEmpty()) Double.NaN else Descriptive.exactMean(values)

    private fun mean0(values: List<Double>): Double = if (values.isEmpty()) 0.0 else Descriptive.exactMean(values)
