 ...
 *
 * The Python had three incompatible definitions (an inventory finding): Type-7 interpolation in the
 * statistics library, an index-based pick in every store/report script, and a rounded nearest-rank
 * in one hypothesis script. The port keeps exactly two:
 *
 * - [type7] is the CANONICAL definition for any new analysis - linear interpolation between order
 *   statistics, the convention numpy's default and most tools share. It is a bit-exact port of
 *   `stats.quantile`, operation order preserved (`lo * (1 - frac) + hi * frac`).
 ...
 *
 * The rounded nearest-rank variant is not ported; the one script that used it is re-derived on
...
object Quantile {
    ...
    /** The Python's minimum n before a quantile's bootstrap CI is shown at all. */
