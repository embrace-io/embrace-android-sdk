package io.embrace.analysis.common.rng
 ...
 * cluster bootstrap and a cluster permutation test, and reproducing the frozen goldens' confidence
 * bounds and p-values to the last digit is what proves the method was carried over intact. That is
 * only possible if every resampling draw is identical, which means the generator must be identical -
 * CPython's Mersenne Twister with its own seeding, its own 53-bit double construction and its own
 * rejection sampling. `kotlin.random.Random` is a different generator (XorWow) with no stability
 * guarantee across stdlib versions.
 *
