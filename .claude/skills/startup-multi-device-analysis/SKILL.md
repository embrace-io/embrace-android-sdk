...
All from the Kotlin `embrace-analysis-*` modules, run from the repo root as `tools/startup <command>`
(`embrace-analysis-cli/README.md` has the full command table; `tools/startup <command> --help` gives the
...
- `tools/startup outlier-factors <pass-dir> <passN-factors.json>` — the external-factor
  ...
  `embrace-analysis-reports/src/main/resources/io/embrace/analysis/reports/sql/`.
