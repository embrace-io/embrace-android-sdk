...
import io.embrace.analysis.records.Derived
import io.embrace.analysis.records.Recipe
import io.embrace.analysis.records.ReferenceSet
import io.embrace.analysis.records.StoreRecord
import io.embrace.analysis.records.StoredTraceHealth
 ...
 *
 * Two defects this implementation guards against:
...
object Ingest {
    ...
    fun canaryFor(instrument: String): String =
        if (instrument == IngestQueries.COMPOSED_INSTRUMENT) COMPOSED_CANARY else instrument
    ...
    fun measure(tp: TraceProcessor, traces: List<Path>, instrument: String): List<Measurement> {
        ...
        val measured = Parallel.map(traces) { trace ->
            tp.withWarmTrace(trace) { target ->
                ...
                val window = TraceReads.parseWindow(target.queryRaw(IngestQueries.windowFor(instrument)).stdout)?.takeIf { it != 0.0 }
        ...
        val signals = tp.withWarmTrace(measured[first].trace) { target ->
            TraceReads.parseSignals(target.queryRaw(IngestQueries.SIGNALS).stdout)
        }
    ...
    fun build(
        ...
        val record = StoreRecord(
            ...
            traceHealth = StoredTraceHealth(tally.traces, tally.lossy, tally.parseErrors, tally.signals.isNotEmpty()),
     ...
     * stored verbatim with its non-profile fields.
     */
    ...
    private fun resolveDeviceKey(ref: ReferenceSet, prov: JsonObject?, options: Options, problems: MutableList<String>): String? {
        // Provenance names the device by key once it has been through the archive hygiene; runs recorded
        // before that carry the serial, which the (hydrated) reference set can still map.
        var deviceKey = options.deviceKey ?: PyJson.strOrNull(prov, "device_key")
        ...
        if (deviceKey == null) {
            problems.add(
                "cannot map this run to a device_key in the reference set " +
                    "(pass --device-key, or ingest a run whose provenance records the device key or serial)",
            )
