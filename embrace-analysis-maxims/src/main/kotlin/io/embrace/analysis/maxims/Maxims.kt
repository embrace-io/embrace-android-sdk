...
object Maxims {
    ...
    fun resolveCell(
        ...
        // Provenance names the device by key once it has been through the archive hygiene; earlier runs
        // carry the serial, which the (hydrated) reference set maps.
        val key = deviceKey.takeUnless { it.isNullOrEmpty() }
            ?: PyJson.strOrNull(meta, "device_key")
            ?: keyForSerial(devices, PyJson.strOrNull(meta, "serial"))
