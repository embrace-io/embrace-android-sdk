 ...
 * A timeout kills the process and surfaces as an
...
open class Adb(
    ...
    open fun run(serial: String?, vararg args: String): Output {
        ...
        cmd.addAll(args)
        val out = try {
            Processes.run(cmd, timeout = timeout)
        } catch (e: Processes.TimedOut) {
            throw IOException("adb ${args.joinToString(" ")} timed out after $timeout" + (serial?.let { " on $it" } ?: ""), e)
        }
        return Output(out.exitCode, out.stdout, out.stderr)
    }
