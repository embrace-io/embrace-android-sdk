...
These are the **frozen spec** for the maxims ledger: regression goldens. They no longer prove
agreement with a second implementation, they prove the code has not changed by accident. A
deliberate rule change is expected to move them, and the diff is the review surface — regenerate
with `tools/startup maxims score` / `render` against these inputs, using the timestamps below, and
read the diff before accepting it.

| item | value |
|---|---|
| Produced | 2026-09-04 |
| Regenerate | run `tools/startup maxims` against the inputs here with the pinned `--now` values (below) |
