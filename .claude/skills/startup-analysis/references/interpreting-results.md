...
- **Class C — scheduler core placement** (heterogeneous silicon). *Signature*: the whole window
  ...
  resource of the `embrace-analysis-reports` module and run by `tools/startup variance`)
...
| E: install/update aftermath | first-launch counter + seconds-since-install/-update | The counter alone conflates fresh installs with post-update firsts; the recency pair separates them and catches aftermath spilling into later launches. |
