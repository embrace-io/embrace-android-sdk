...
/**
 * Sweep a source tree for references to one machine or one author - things that are true where the
 * code was written and false everywhere else. Ported from the production repo's `check_leaks.py`,
 * whose design this keeps: a checked-in layer of GENERIC patterns describing the *shape* of a local
 * reference, and a git-ignored layer of PERSONAL tokens naming one author's actual values.
 *
 ...
 *
 * The engine knows nothing about any particular repository's layout: [scan] takes the [Roots] to walk,
 * so the same checker can sweep any tree. This repository's own roots are supplied by the command that
 * runs it, not defined here.
 *
 * A line containing `local-refs:allow` is skipped entirely, for the case that legitimately looks like
 * one. `--allow REGEX` suppresses one matched text for a single run without editing anything.
 */
object LocalRefsCheck {

    enum class Scope { EVERYWHERE, SOURCE }

    /**
     * One local-reference shape, with the samples that prove it still works. The samples live on the
     * pattern rather than in a table beside it, so a pattern cannot be added or edited without its own
     * test moving too.
     */
    class LocalRef(
        val name: String,
    ...
    val PATTERNS: List<LocalRef> = listOf(
        LocalRef(
            "home-path",
    ...
    /**
     * Where to look, relative to the repository root passed to [scan]: [source] paths are held to every
     * pattern, [data] paths only to the personal ones, and [skipInSource] names directories the source
     * walk steps over because the data walk covers them under their own scope.
     */
    data class Roots(
        val source: List<String>,
        val data: List<String>,
        val skipInSource: Set<String> = emptySet(),
    )

    /**
     * Scan [repo] over [roots]. [extra] paths are scanned in addition, as source, whatever their
     * extension. [localTokens] are matched case-insensitively as plain substrings; [allow] suppresses a
     * hit whose matched text matches any of them.
     */
    fun scan(
        repo: Path,
        ...
        // The source pass walks past what the data pass then covers under its own scope.
        roots.source.forEach { rel ->
            collect(repo.resolve(rel), Scope.SOURCE, SOURCE_EXTENSIONS, targets, SKIP_DIRECTORIES + roots.skipInSource)
        }
        roots.data.forEach { rel -> collect(repo.resolve(rel), Scope.EVERYWHERE, DATA_EXTENSIONS, targets, SKIP_DIRECTORIES) }
    ...
    fun localTokenFile(repo: Path): Path? {
        System.getenv("LOCAL_REFS_TOKENS")?.let { env ->
            val p = Path.of(env)
    ...
    fun selfTest(): List<String> = PATTERNS.flatMap { pattern ->
        buildList {
            if (!pattern.regex.containsMatchIn(pattern.positive)) {
                add("${pattern.name} did not match its positive sample: ${pattern.positive}")
            }
            if (pattern.regex.containsMatchIn(pattern.negative)) {
                add("${pattern.name} matched its negative sample: ${pattern.negative}")
            }
        }
    }
    ...
    private fun scanLines(
        ...
        lines.forEachIndexed { index, line ->
            ...
            PATTERNS.forEach { pattern ->
                if (pattern.scope == Scope.SOURCE && scope != Scope.SOURCE) return@forEach
                pattern.regex.findAll(line).forEach { match ->
                    if (allow.none { it.containsMatchIn(match.value) }) {
                        findings.add(Finding(rel, index + 1, pattern.name, match.value))
                    }
                }
            }
    ...
    /** The records keep evidence in archives, so a local reference can hide inside one; read them without unpacking. */
     ...
     * and its test hold a sample of every local-reference shape by construction, so they are the one place the
    ...
    private val EXEMPT_FILES = setOf(
        ...
        "LocalRefsCheck.kt",
        "LocalRefsCheckTest.kt",
    )
    private const val EXEMPT_PREFIX = "reference-set"

    private const val LOCAL_FILE = ".local-refs-tokens"
    private const val ALLOW_MARKER = "local-refs:allow"
