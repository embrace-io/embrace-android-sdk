 ...
 * committed records root (`maxims/` under [RecordsRoot.dir]). Recording a run also packs its EVIDENCE -
 * per-pass datasets, harness output, log, provenance naming the device by key - into
 * `campaigns/<run-id>.zip` in the same root: the traces they were derived from are wiped by the next
 * benchmark, so the datasets are the durable evidence behind the ledger, and an archive that is written
 * once and never edited is one compressed file rather than a directory to version. Raw members and the
 * serial stay with the run directory under the machine-local root.
 */
    ...
    private class Score : CliktCommand(name = "score") {
        ...
        private val records by option(
            ...
            help = "records root whose campaigns/<run-id>.zip keeps the run's datasets and log once it is recorded " +
                "(default: ${RecordsRoot.REL} under the repo); 'none' to keep nothing",
        )
        ...
        override fun run() {
            ...
            val text = try {
                MaximsRunner.score(
                    ...
                    serials = DeviceSerials.load(DeviceSerials.file()),
                )
