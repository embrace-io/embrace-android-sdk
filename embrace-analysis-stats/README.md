...
- **`CPythonRandom` exists instead of `kotlin.random.Random`** because the method of record is a
  seeded cluster bootstrap and permutation test whose goldens were produced with CPython's Mersenne
  Twister; a bit-identical generator is what makes them reproducible to the last digit.
