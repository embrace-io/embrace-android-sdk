# Golden fixtures for the startup-analysis toolchain

Frozen reference outputs on fixed inputs: the specification for this tooling's behaviour. There is
no generator for these files - they cannot be regenerated and no longer prove agreement with a
second implementation. A golden that moves means the code changed, and the diff must be read before
deciding whether that change is intended. Everything below - the serialization conventions, the RNG
contract, the per-file tolerances, the recorded inconsistencies - documents required behaviour, not
...
| Regenerate | not possible - these files are the specification; there is no generator |
...
section, keyed by slice name) but nothing in the suite consumes them, so no golden is derived from
them.

## Serialization conventions (apply to every JSON here)

* Written with `json.dumps(obj, sort_keys=True, indent=1)` plus a trailing newline. JSON produced by
  a `--json`/`--emit` flag was re-loaded and re-written with the same options - key ORDER changed,
  semantics did not.
...
## The RNG contract (needed for bit-exact bootstrap/permutation parity)
...
`rng.json` pins all four so the RNG implementation can be validated in isolation before any statistic is.

## Files

### `rng.json`
* **Inputs:** `random.Random(12345)` from the stdlib; a FRESH generator for each of the four
...
* **Inputs:** the statistics module's public surface, plus `_ndtri`, exercised on inputs recorded
  in the file (`inputs.a_clusters`, `inputs.b_clusters` - 5 clusters x 8 values, exact
  ...
  defaults: `seed=12345`, `resamples=10000`, `alpha=0.05`.
...
* **Compare / tolerance:**
  * bit-exact: `quantile`, `quantile_support`, `quantile_ci_trustworthy`, `cliffs_delta`
    ...
    `n_for_quantile`, `n_for_exceedance_rate` - `math.ceil` outputs; if an implementation is off by
    exactly 1 the pre-ceil value sits within an ulp of an integer and needs a closer look, not a
    tolerance), bootstrap `diff` and `ci` for `statistic in {median, quantile}` (pure `quantile()`
    arithmetic over the same draws), permutation `p`/`observed`/`min_attainable_p`
    (`p = (hits+1)/(resamples+1)` so `hits` must match exactly), every `reason` string, `available`
    flags, `compare.n`, `compare.clusters`, `compare.median`, `compare.effect_size`,
    `compare.quantiles.*.support`.
  * relative 1e-9: `icc_oneway`, `design_effect.{icc,deff,n_effective}` - Python's
    `statistics.mean` sums as exact rationals, an implementation that sums doubles will differ at
    this precision; `cluster_bootstrap_diff` with `statistic="mean"` for the same reason;
    `tost_equivalence.ci_pct`; `compare.median_diff_pct`.
...
### `stats_store.json`
* **Inputs:** the real longitudinal store. Each arm's `windows_ms` (200 values, already rounded to
  3 dp at ingest) is chunked IN FILE ORDER into 10 clusters of 20; the chunks are recorded in
  `pairs.*.a_clusters` / `b_clusters` so the test is self-contained. Direction is always `b - a`
  (newer minus older).
...
* **Compare / tolerance:** same rules as `stats_synthetic.json`. With n=200 per arm the p90 and p95
  ...
  `1/10001 = 0.00019998...` (0 hits) - an implementation that gets `hits > 0` has a real bug, not a
  rounding difference.

### `derive_store.json`
* **Inputs:** `derive(windows_ms)` (the aggregate computation below) applied to every record of both
  stores, keyed by `run_id`, with the record's stored `derived` alongside. `edge_cases` covers
  n = 0, 1, 3, 4, 5 (the `iqr` branch switches at n >= 4).
...
* **NOTE:** `derived` here does NOT equal `stored_derived` (differences in the 4th decimal). The
  ...
  3 dp afterwards. An implementation of `derive` must match `derived`, not `stored_derived`.

### `trend_store.json`, `trend_store.stdout.txt`
* **Inputs:** `claude-output/longitudinal/store.jsonl` (the real longitudinal store) with
  `--baseline-runs 3` (the default).
* **Result:** the JSON is `{}` - every real series has exactly one run, so the run `continue`s
...
* **Inputs:** `claude-output/longitudinal/sweep-store.jsonl`, same `--baseline-runs 3`. Also `{}`.
  Exercises the `"?"` fallbacks in `group_key()` (three records have `sdk_version: null` and null
  `build_type`/`compile_state`) and the absence of a version-comparison block when no cell has two
  versions.

### `trend_synthetic.json`, `trend_synthetic.stdout.txt`, input `inputs/trend-store.jsonl`
* **Inputs:** a synthetic 14-record store (windows from `centre + 0.3*((i*37) % 21 - 10) +
  (6.0 if i % 20 == 19)`, rounded to 3 dp, with an optional fat tail `+12.0` on every 10th value;
  each record's `derived` is `derive` applied to its own windows).
...
* **Inputs:** `goldens/inputs/corpus.jsonl` (below).
* **Input:** 6 hand-built contribution records following the submission allowlist
...
* **Inputs:** `<plan-example.json>` with `--emit goldens/matrix_plan.json`. The plan ships
  placeholders (`<oldest supported release>`, `<serial>`) and `passes: 4, iterations: 50`; both flow
  straight into cell ids and the `4x50 is weaker` warning.
...
* **Inputs:** env `LITTLE_CPUS=0,1,2,3` set explicitly (the default, but pinned).
  * hypothesis tests and factors report: `goldens/inputs/campaign-a`
  * cross-device sections: `synth-mid=goldens/inputs/campaign-a synth-entry=goldens/inputs/campaign-b synth-mid=goldens/inputs/campaign-b`
...
* **Inputs** (all under `inputs/`; formulas are closed-form in pass `p` and iteration `j`):
  * `campaign-a/pass{1,2,3}.json` - the per-iteration variance dataset schema:
    ...
    (16 default sections + `emb-power-service-registration` + `emb-snapshot-session`).
  * `campaign-a/pass{1,2,3}-factors.json` - the outlier-factors dataset schema: `window_ms, eff_mhz,
  ...
  * `campaign-b/pass1.json`, `campaign-b/pass3.json` - the same formulas at scale 2.5 with
    `emb-record-startup` dropped (prints `--`), and NO pass2 (exercises the `continue`-on-missing-pass
    path in cross-device sections; hypothesis tests would `break` there instead).
* **Compare:** these analyses only print, so the golden is the text. Compare line-by-line after
  ...
  before calling it a bug. Pearson `r` values are printed at 2 dp; an implementation using naive
  double sums instead of `statistics.mean` will agree at that precision.

### `x37_legs.json`
* **Inputs:** the 22 `*-windows.json` files in `claude-output/2026-08-26-x37-engine-tail/`
  ...
  (`quantile(sorted(w), p)`), and the raw `windows_ms`. Then `compare(a, b, "compat",
...
## Behaviour NOT exercised by these goldens, and why

* Everything that needs `trace_processor`, `adb`, gradle, git-mutation or the network: `analyze`,
  `variance` (only its JSON *schema* is reproduced), `outlier-factors` (schema only), `ingest`'s
  main path (only `derive` is pure), `submit`'s device-provenance collection (uses `adb` and a
  secret salt; the corpus records were hand-built to its allowlist), `probe`, `fleet-campaign`,
  `cell-runner`, `compat-patch`, `matrix-report`, `reference-set`, `trace-health`, `verify-arms`,
  `serve-trace`.
* The borrowed-state signal/marker-file behaviour and `artifact-sync` (hard-coded absolute
  manifest path, writes to `claude-output/artifact-manifest.json`) - side-effecting by nature and
  outside the numeric core.
* Every public statistics function is exercised, plus `_ndtri`.
* Only `--baseline-runs 3` (the default) is frozen for `trend`.

## Inconsistencies noticed (recorded, NOT fixed)

1. **Three quantile definitions.** the stats module's `quantile` is Type-7 interpolation; `ingest`,
   `trend`, `reproducibility` and `matrix-report` use the element pick
   `values[min(n-1, int(p*n))]`; `hypothesis-tests`' `pctl` uses `values[min(n-1, int(round(p/100*(n-1))))]`
   ...
   position 179.1. All three must be kept, per call site.
2. **Store `derived` vs `derive(stored windows)` disagree** (4th decimal) because ingest
...
3. **`sweep-store.jsonl` is only partly usable:** 3 of its 4 records (`8.3.0__flagship-a`,
   ...
   version. `trend` groups them under `sdk=? build=? compile=?`. Only `8.3.0__entry-a` is a
   proper record, which is why it is the one used in `stats_store.json`.
4. **`reproducibility` reads `derived.pass_medians`**, which `derive` never produces (and
   `submit` copies `derived` verbatim from the store). With the documented pipeline
   `own_spread_pct` is always `None`, the tolerance is always the 5% floor and `shape_of`
...
5. **Tolerance derived from the data can hide a median disagreement:** in the Galaxy A14 cell a
   ...
   `agree`; the reminder "never widen tolerance to force agreement" is undercut by the
   tolerance rule itself. Design observation, not a code bug.
6. **`HUNT_FIELDS` includes `gate_temp_c` and `tool_versions`** which `submit` never writes;
   they can never appear in `candidates` (confirmed: absent from the golden).
7. **`hypothesis-tests` prints `slow iterations (delta > +4 ms)`** but the effective threshold is
   `max(4.0, 0.10 * pass median)` (5.1 ms for synthetic pass 2). Label and logic disagree.
8. **`trend --json` and `matrix-plan --emit` write `indent=1` without `sort_keys`**, unlike
   `artifact-sync`; re-serialized here for uniformity.
9. **Formatting overflow in `matrix-plan` stdout:** the device column has no separator for the
   8-character device `flagship` (`flagshipcombo:bad-day`, line 15). Cosmetic; the golden keeps
   it as printed.
10. **`cross-device-sections`' header columns collide** for labels longer than ~5 chars (line 3
    and 7 of its golden). Cosmetic.
11. **Flagship 9.1.0 has n=198** and flows through `trend`'s version comparison without
    comment; ingest's truncation guard is `< 0.9 x 200 = 180`, so 198 passed. Any code that
    assumes 10x20 chunking must check n first (as `stats_store.json` does).
12. **`derive` and `quantile(…, 0.5)` agree on the median** (both average the two middle values for
    even n), so `median` can be implemented as `quantile(sorted, 0.5)` without loss - but
    `derive`'s p90/p95 cannot (see 1).
13. **`pearson` is duplicated** in `variance`, `hypothesis-tests` and `factors-report` and returns
    `nan` for zero variance; none of the goldens hit that branch.
