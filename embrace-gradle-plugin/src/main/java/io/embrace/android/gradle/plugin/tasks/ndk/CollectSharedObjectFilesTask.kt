...
abstract class CollectSharedObjectFilesTask @Inject constructor(
    ...
    private fun getNativeSharedObjectFiles(): Array<File> {
        val files = architecturesDirectoryForNative.singleOrNull()?.listFiles() ?: emptyArray()

        if (files.isEmpty()) {
            // TODO: is this an error? does every native project have .so files?
            error("Native shared object files not found")
        }
