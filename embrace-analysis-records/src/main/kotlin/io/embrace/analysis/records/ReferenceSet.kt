...
data class ReferenceDevice(
    /**
     * Absent from the committed file: a serial names a handset on one machine, so it lives in the local
     * device-serial map and is filled in by `DeviceSerials.hydrate` when a reader needs it. Nullable
     * rather than removed so reference sets written before the split still read.
     */
