 ...
 * this is checked against hand-derived expectations on a synthetic run directory. Departure: the
 * default window slice is `emb-sdk-start`, with `composed` for pre-9.2.0 cells - never
...
object MatrixReport {
    ...
    fun windowReader(tp: TraceProcessor, slice: String): (Path) -> Double? = { trace ->
        TraceReads.parseWindow(tp.queryRaw(IngestQueries.windowFor(slice), trace).stdout)
    }
