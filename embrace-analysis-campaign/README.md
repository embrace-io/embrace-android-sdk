...
| `CellRunner` | one matrix cell: invariants, factor state, passes, provenance | a failed invariant is a STOP, not a warning; writes `cell-state.json`; its default run directory sits under the machine-local root (`RecordsRoot.localRuns`), not the scratch directory, so a cell survives a scratch clean-up |
...
| `Reproduce` | a recorded run's provenance turned back into the commands that would produce it again | pins the app commit, the SDK coordinate, the device (serial looked up from the local device-serial map) and the method/shape; refuses (no steps) when the run's evidence class is irreproducible |
