...
- **`_shared/local/`** (gitignored, never scratch) - the machine-local root beside the records: the
  device-serial map, the raw members culled from evidence archives, run directories and rendered pages.
  Identifying or re-derivable, so never committed; `data/` under it is the only copy of what it holds.
