...
class TraceProcessorTest {
    ...
    fun `live - the native engine reproduces the launcher's rows for every query on every fixture trace`() {
        ...
        val sqlByQuery = mapOf(
            "startup_metrics" to Queries.STARTUP_METRICS,
            "init_window_sched" to Queries.INIT_WINDOW_SCHED,
            "variance_metrics" to Queries.VARIANCE_METRICS,
            "outlier_metrics" to Queries.OUTLIER_METRICS,
            "foreign_gc_overlap" to Queries.FOREIGN_GC_OVERLAP,
            "health" to TraceHealth.sql(),
            "window_emb_sdk_start" to Queries.window("emb-sdk-start"),
            "window_composed" to Queries.COMPOSED_WINDOW,
            "signals" to Queries.SIGNALS,
        )
