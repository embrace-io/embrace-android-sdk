...
- A record names its device by key and only by key. The key-to-serial map is machine-local
  (`_shared/local/data/device-serials.json`); ingest hydrates the reference set from it, so a run whose
  provenance carries a serial still resolves on the machine that owns the handset, while the committed
  store and reference set carry no identifier.
