...
internal class SessionSanitizerFacade(
    private val sessionMessage: SessionMessage<Session>,
    private val components: Set<String>
) {

    fun getSanitizedMessage(): SessionMessage<Session> {
        InternalStaticEmbraceLogger.logger.logDeveloper(
            "SessionSanitizerFacade",
            "getSanitizedMessage"
        )
        val sanitizedSession = SessionSanitizer(sessionMessage.data, components).sanitize()
