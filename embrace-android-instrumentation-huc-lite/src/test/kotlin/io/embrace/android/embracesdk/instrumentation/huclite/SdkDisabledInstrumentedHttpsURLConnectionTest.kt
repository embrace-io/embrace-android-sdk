...
internal class SdkDisabledInstrumentedHttpsURLConnectionTest {
    private lateinit var harness: HucTestHarness

    @Before
    fun setup() {
        harness = HucTestHarness(sdkEnabled = false)
    }
    ...
    @Test
    fun `no request recorded when the wrapped call throws an exception`() = harness.runTest {
        every { mockWrappedConnection.connect() } throws FakeIOException()
    ...
    fun `getResponseCode delegates to wrapped connection`() = harness.runTest {
        ...
        verify(exactly = 1) { mockWrappedConnection.responseCode }
    ...
    @Test
    fun `getResponseMessage delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getHeaderField with int delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getHeaderFieldKey delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getErrorStream delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getHeaderField with string delegates to wrapped connection`() = harness.runTest {
        every { mockWrappedConnection.getHeaderField(FAKE_VALUE) } returns FAKE_VALUE

        val result = instrumentedConnection.getHeaderField(FAKE_VALUE)

        assertEquals(FAKE_VALUE, result)
        verify(exactly = 1) { mockWrappedConnection.getHeaderField(FAKE_VALUE) }
        assertNoRequestRecorded()
    }

    @Test
    fun `getHeaderFields delegates to wrapped connection`() = harness.runTest {
        val mockHeaders = mutableMapOf(FAKE_FIELD_NAME to mutableListOf(FAKE_VALUE))
    ...
    @Test
    fun `getInputStream delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getContent delegates to wrapped connection`() = harness.runTest {
    ...
    @Test
    fun `getContent with classes delegates to wrapped connection`() = harness.runTest {
