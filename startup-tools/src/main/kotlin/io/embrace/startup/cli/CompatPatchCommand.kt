...
/** `compat-patch` - the former `compat_patch.py`: per-version app-side compatibility patches plus the version pin. */
class CompatPatchCommand : CliktCommand(name = "compat-patch") {
    ...
    override fun run() {
        ...
        val v = version
        if (v == null || !apply) {
            echo("use --version X --apply, or --revert-all", err = true)
