...
internal class ExperimentTrackingServiceImplTest {
    ...
    fun `re-tracking a known id with a different variant is dropped`() {
        service.track(
            listOf(
                ...
                TrackedData.featureFlag(id = "flag1", variant = "on", startTimeMs = 100L),
            ),
        )
        ...
        service.track(
            listOf(
                TrackedData.experiment(id = "id1", variant = "v2", startTimeMs = 200L),
                TrackedData.featureFlag(id = "flag1", variant = "off", startTimeMs = 200L),
            ),
        )
        service.assertRecordState("e:id1:v1:100;f:flag1:on:100")
    ...
    fun `untrack closes a feature flag record`() {
        service.track(
            listOf(
                TrackedData.featureFlag(id = "flag1", variant = null, startTimeMs = 100L),
                TrackedData.featureFlag(id = "flag2", variant = "on", startTimeMs = 100L),
            ),
        )
        service.untrack(ExperimentKind.FEATURE_FLAG, listOf("flag1", "flag2"), 200L)
        service.assertRecordState("f:flag1::100:200;f:flag2:on:100:200")
    }
    ...
    fun `an experiment and a feature flag sharing an id are two independent records`() {
        ...
        service.track(
            listOf(TrackedData.featureFlag(id = "id1", variant = "on", startTimeMs = 150L)),
        )
        service.assertRecordState("e:id1::100;f:id1:on:150")

        // untrack matches on kind as well as id, so only the feature flag record is closed
        service.untrack(ExperimentKind.FEATURE_FLAG, listOf("id1"), 200L)
        service.assertRecordState("e:id1::100;f:id1:on:150:200")
    }
    ...
    fun `ids and variants are stripped of ascii whitespace before validation, identity, and serialization`() {
        ...
        service.track(
            listOf(TrackedData.featureFlag(id = " f;1\n", variant = "\ton ", startTimeMs = 100L)),
        )
        // the stripped id "a:b" passes the 3-char limit, is stored unescaped, and is escaped only at serialization
        service.assertRecordState("e:a%3Ab:v1:100;f:f%3B1:on:100")

        // untrack matches on the stripped id
        service.untrack(ExperimentKind.EXPERIMENT, listOf("a:b "), 200L)
        service.untrack(ExperimentKind.FEATURE_FLAG, listOf(" f;1"), 200L)
        service.assertRecordState("e:a%3Ab:v1:100:200;f:f%3B1:on:100:200")
    }
    ...
    fun `null, empty, and whitespace-only variants are identical and absent in the output`() {
        service.track(
            listOf(
                ...
                TrackedData.featureFlag(id = "flag1", variant = null, startTimeMs = 100L),
                TrackedData.featureFlag(id = "flag2", variant = "", startTimeMs = 100L),
                TrackedData.featureFlag(id = "flag3", variant = " \t", startTimeMs = 100L),
            ),
        )
        service.assertRecordState("e:id1::100;e:id2::100;e:id3::100;e:id4::100;f:flag1::100;f:flag2::100;f:flag3::100")
    }
    ...
    fun `a variant longer than the max length is dropped silently`() {
        ...
        service.track(
            listOf(
                ...
                TrackedData.featureFlag(id = "flag1", variant = "123456", startTimeMs = 100L),
            ),
        )
        service.assertRecordState(null)
    }
    ...
    fun `making multiple calls in bulk results in one attribute write`() {
        service.bulkModify(
            listOf(
                ...
                ExperimentApiCall.Track(
                    listOf(TrackedData.featureFlag(id = "f", variant = "on", startTimeMs = 150L)),
                ),
                ExperimentApiCall.Untrack(ExperimentKind.FEATURE_FLAG, listOf("f"), 400L),
            ),
        )
        service.assertRecordState("e:a::100:300;e:b::200;f:f:on:150:400")
        assertEquals(1, experimentAttributeWriteCount())
    }
    ...
    fun `serialization order follows tracking order irrespective of kind`() {
        ...
        service.track(
            listOf(TrackedData.featureFlag(id = "id2", variant = null, startTimeMs = 200L)),
        )
        service.track(
            listOf(TrackedData.experiment(id = "id3", variant = null, startTimeMs = 300L)),
        )
