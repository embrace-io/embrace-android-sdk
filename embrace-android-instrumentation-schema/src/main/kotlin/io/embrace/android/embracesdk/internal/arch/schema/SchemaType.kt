...
sealed class SchemaType(
     ...
     *
     * If the string alone does not identify a value uniquely because they are generated by the SDK and the string form
     * overlaps with values that are derived from app data, those values can implement [SystemStateValue] so that its
     * value type is recorded alongside them to fully represent their uniqueness.
     */
    abstract class State<T : Any>(
        ...
        override val schemaAttributes: Map<String, String> = buildMap {
            ...
            valueType(initialValue)?.let { valueType ->
        ...
        companion object {
            /**
             * The value type to record alongside the given state [value], or null if none should be recorded. Only a
             * [SystemStateValue] has a value type.
             */
            fun valueType(value: Any): String? {
                return if (value is SystemStateValue) {
