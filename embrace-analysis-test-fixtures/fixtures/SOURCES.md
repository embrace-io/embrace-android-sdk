...
- `legs/x37.zip` <- 23 per-leg window files from the engine-tail experiment, kept in the records root
  under `experiments/`
- `goldens/` <- frozen CLI and statistics outputs; see its own MANIFEST for provenance (the tests
  read the JSON, never run a generator)
- `maxims/` <- the closed-form maxims fixture; see its own MANIFEST
- `trace-goldens/data.zip` <- the frozen trace-layer outputs; see its own MANIFEST
