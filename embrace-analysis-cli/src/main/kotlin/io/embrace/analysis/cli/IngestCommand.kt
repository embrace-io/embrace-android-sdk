...
class IngestCommand : CliktCommand(name = "ingest") {
    ...
    override fun run() {
        // The committed reference set names devices by key; the local serial map lets a run whose provenance
        // still carries a serial resolve to its key on the machine that owns the handset.
        val ref = DeviceSerials.hydrate(
            StartupJson.decodeFromString(ReferenceSet.serializer(), Files.readString(referenceSet)),
            DeviceSerials.load(DeviceSerials.file()),
        )
        val instrument = ref.recipe.instrument
        val provenance = Provenance.load(runDir)
