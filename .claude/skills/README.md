...
| `no-local-references.md` | the two-scope leak sweep (`tools/startup check-leaks`), which must pass before any skill edit is done |
| `records/` | everything the tooling has learned and must keep - see its own README |

## Where the output goes

The tooling is `tools/startup` (the `startup-tools` Gradle module); these skills are its instructions,
not its code.
...
Run `tools/startup check-leaks` before calling any skill edit done - it fails the build on a personal
