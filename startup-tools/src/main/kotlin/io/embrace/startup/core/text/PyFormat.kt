package io.embrace.startup.core.text
...
object PyFormat {
    ...
    /**
     * Python's `statistics.mean` on floats: an exactly rounded result (it sums as rationals). The
     * `DECIMAL128` division is exact to 34 digits, far beyond a double, so the rounding to double
     * matches Python's in every case a report can show.
     */
