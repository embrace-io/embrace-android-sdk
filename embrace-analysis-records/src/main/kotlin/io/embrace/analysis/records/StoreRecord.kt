...
data class StoreRecord(
     ...
     * were unaffected.
     * Ingest refuses to WRITE a null version; this field is nullable so it can still READ one.
     */
    ...
    @SerialName("trace_health") val traceHealth: StoredTraceHealth? = null,
 ...
 * `values[min(n-1, floor(p*n))]`, not the Type-7 interpolation the statistics library uses - this
 * definition is kept for stored fields under the name "legacy index" so existing records remain
