...
Every archive under `../experiments/` is an argument, and this is the conclusion each one supports; a
campaign archive is cited by the ledger (`ledger-runs.json`) and indexed in `../campaigns/index.json`, so
it needs no entry here unless a finding rests on it alone. `tools/startup records check-links` fails when
an archive is missing from this list or a citation names an archive that does not exist.

| archive | what it holds | supports |
|---|---|---|
| `2026-08-11-a14-5x50-corroboration` | the verdict sheet, the T1 `system_server` GC quantification and the outlier-first factors report from the 5 x 50 corroboration campaign on `mid-a` (its datasets are `campaign-2026-08-11`) | the Samsung compile-state toggle and its asymmetric section inflation; core placement as a correlate, not a cause; the first-launch config bimodality; concurrent `system_server` CPU as the outlier mechanism; the three outlier types |
| `probes-2026-08-12` | the topology probe logs of `mid-a` and an induced-pressure probe on `flagship-a` | the eight-identical-A55 topology behind the cluster-0 finding; the induced-churn causal test of concurrent CPU |
| `2026-08-14-attribute-studies` | the grading runs behind the attribute table: IO, memory, thermal, recency and the 17 section durations against trace truth | every row of the telemetry-attributes table except the compile-state pair |
| `2026-08-16-x25-artifacts` | the 10 x 20 version comparison, the salvaged aggregates of the destroyed 4 x 50 store, and the two reference sets (serials removed) | the monotonic 8.3.0 → 9.2.0 improvement; pass count as series-defining; the unexplained 4 x 50 → 10 x 20 level shift |
| `2026-08-17-x27-artifacts` | the tap-overhead ABBA campaign on `flagship-a` (tap on against tap off, 400 launches) | attribute-joined campaigns are admissible: the tap's window cost is bounded at ≤1.1 ms and its TTID cost at ≤5.3 ms |
| `2026-08-17-join-campaign-artifacts` | the per-launch attribute-to-window pairs (`pairs.jsonl`) of the first tapped campaign | the input to the attribute prediction test below |
| `2026-08-18-x29-analysis` | the analysis of those pairs: rank correlations within phase, three attribute populations kept apart | `thermal-headroom-pct` as the one attribute shown to predict a slow init; why in-window counters cannot be correlated against the window |
| `2026-08-17-x32-engine-artifacts` | the Kotlin-versus-compat OTel engine A/B on `flagship-a`, propagation verified from trace slices | the flagship's −4.3%, and why a flagship-only reading of the engine lever was wrong by 4x |
| `2026-08-25-x34-attribution` | the 4-pass section attribution of 9.0.0 → 9.2.0 on all four devices | the 19 sections outside the composed window; the rename boundary; the two sections that carry the gain |
| `2026-08-25-x35-attribution-10x20` | the same question at the store's 10 x 20 shape, with containment measured on every trace | `mid-b`'s shortfall in `post-services-setup`; the containment tree; pass count changing what is measured |
| `2026-08-26-rename-map` | the containment tree rebuilt for both versions and the residual of `emb-modules-init` | `emb-modules-init`'s growth as a phase shift inside the rename boundary, not concealed cost |
| `2026-08-26-x37-arm-reality-check` | the two-APK comparison that showed the engine-tail campaign's arms were the same SDK | the void verdict on X37 |
| `2026-08-26-x37-engine-tail` | the per-leg window arrays of the void engine-tail campaign | the engine's tail is unmeasured; the propagation gate as a precondition of any flag A/B |
| `2026-08-26-drivers` | the README of the ad hoc drivers that campaign used, preserved before the scratch directory was purged (the drivers themselves were not kept) | the propagation-gated redo design in the open question on the engine tail |
| `2026-09-03-first-session-fix` | the three first-session A/Bs on `mid-b`, cohort-verified, with the class-load deobfuscation | the restore-path finding; the create-path serializer cost; the class-load burst threshold of 80 |
...
- **2026-09-05** — Ten more campaigns scored, all on `mid-b` except the Go device's four-pass mini
  ...
  `passN-factors.json` were produced from the traces before those are lost (the three `mid-b` tooling
  validation runs of 26 August, the four user-session arms of the first-session fix, the paired `mid-b`
  runs of 5 September). The ledger is now rebuilt end to end by `tools/startup records rebuild-ledger`,
  which scores every run named in `maxims/ledger-runs.json` with that entry's cell overrides.
...
- **2026-09-05** — **The three scoping gaps are closed.** A
  ...
  arm with none. The goldens under `embrace-analysis-test-fixtures` are the specification of every
  maxim's wording and threshold: a golden that moves means the code changed, and the diff is read
  before it is accepted. Records maintenance is `tools/startup records pack` and `records
  rebuild-ledger`; the rebuild's run list lives in `ledger-runs.json` beside the ledger, so adding a
  campaign is a data edit rather than a code one.
